...
public:
   ...
   */
   PtEtaPhiM4D(Scalar pt, Scalar eta, Scalar phi, Scalar mass) : fPt(pt), fEta(eta), fPhi(phi), fM(mass)
   {
      RestrictPhi();
      if (fM < 0)
         RestrictNegMass();
   ...
   */
   ...
   explicit PtEtaPhiM4D(const CoordSystem &c) : fPt(c.Pt()), fEta(c.Eta()), fPhi(c.Phi()), fM(c.M())
   {
      RestrictPhi();
   }
   ...
   /**
      copy constructor
    */
   PtEtaPhiM4D(const PtEtaPhiM4D &v) : fPt(v.fPt), fEta(v.fEta), fPhi(v.fPhi), fM(v.fM) {}
   ...
   */
   Scalar M2() const { return (fM >= 0) ? fM * fM : -fM * fM; }
...
public:
   ...
   */
   ...
   {
      return fPt == rhs.fPt && fEta == rhs.fEta && fPhi == rhs.fPhi && fM == rhs.fM;
   }
