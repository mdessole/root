...
public:
   ...
   */
   explicit RotationZ(Scalar angle) : fAngle(angle), fSin(mysin(angle)), fCos(mycos(angle)) { Rectify(); }
   ...
   */
   RotationZ operator*(const RotationZ &r) const
   {
      ...
      ans.fAngle = (2.0 * M_PI) * (x + myfloor(.5 - x));
