...
public:
   ...
   Scalar X() const { return fRho * mycos(fPhi); }
   Scalar Y() const { return fRho * mysin(fPhi); }

   Scalar Mag2()  const { return fRho*fRho + fZ*fZ;   }
   Scalar R() const { return mysqrt(Mag2()); }
   Scalar Perp2() const { return fRho*fRho;           }
   Scalar Theta() const { return (fRho == Scalar(0) && fZ == Scalar(0)) ? Scalar(0) : myatan2(fRho, fZ); }
